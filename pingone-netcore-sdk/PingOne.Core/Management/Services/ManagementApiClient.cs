...
namespace PingOne.Core.Management.Services
{
    public class ManagementApiClient : IManagementApiClient
    {
        ...
        public async Task<IEnumerable<User>> FindAllUserAsync(string userName)
        {

            var response = await _httpClient.GetAsync($"users/?filter=email sw \"{userName}\" or username sw \"{userName}\"");

            var jsonResult = await response.Content.ReadAsStringAsync();
            var parsedResult = JObject.Parse(jsonResult);
            return parsedResult["_embedded"]["users"].ToList().Select(x=> x.ToObject<User>());
        }
